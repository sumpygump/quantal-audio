...
struct UnityMix : Module {
    ...
    void process(const ProcessArgs &args) override {
        ...
        if (unconnect) {
            ...
            const polysignal mix = mixChannels(0, 5);
...
struct UnityMixWidget : ModuleWidget {
    explicit UnityMixWidget(UnityMix *module) {
        setModule(module);
        setPanel(
            createPanel(
        ...
        // Connect switch
        addParam(createParam<CKSS>(Vec(RACK_GRID_WIDTH - 7.0f, 182.0), module, UnityMix::CONNECT_PARAM));

        // Group A
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 50.0), module, UnityMix::CH_INPUT + 0));
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 78.0), module, UnityMix::CH_INPUT + 1));
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 106.0), module, UnityMix::CH_INPUT + 2));
        addOutput(createOutput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 148.0), module, UnityMix::CH_OUTPUT + 0));

        // Group B
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 222.0), module, UnityMix::CH_INPUT + 3));
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 250.0), module, UnityMix::CH_INPUT + 4));
        addInput(createInput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 278.0), module, UnityMix::CH_INPUT + 5));
        addOutput(createOutput<ThemedPJ301MPort>(Vec(RACK_GRID_WIDTH - 12.5f, 320.0), module, UnityMix::CH_OUTPUT + 1));
    }
};
